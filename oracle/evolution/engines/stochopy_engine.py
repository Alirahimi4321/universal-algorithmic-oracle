"""Stochopy-based optimization engine using stochopy.optimize.minimize."""
...
try:
    ...
    HAS_STOCHOPY = False
    logger.info("stochopy not available")


class StochopyEngine:
    """Optimization engine using stochopy (CMA-ES, DE, PSO)."""

    METHODS: list[str] = ["cmaes", "pso", "de"]

    def __init__(self, config: dict | None = None) -> None:
        config = config or {}
        self.population_size: int = config.get("population_size", 30)
        self.max_generations: int = config.get("max_generations", 100)
        self.method: str = config.get("method", "de")
    ...
    def evolve(self, entropy_packet: dict, generations: int | None = None) -> list[Chromosome]:
        ...
        try:
            x0 = np.random.uniform(-1.0, 1.0, n_genes)
            result = stochopy_minimize(
                objective, bounds,
                x0=x0,
                method=self.method,
                options={"maxiter": max_gen, "popsize": min(self.population_size, 20)},
            )
        except Exception as e:
            logger.warning("stochopy minimize failed: %s", e)
            return self._fallback_evolve(entropy_packet, max_gen)
